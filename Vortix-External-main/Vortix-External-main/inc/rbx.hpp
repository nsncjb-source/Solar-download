...
namespace RBX
{
    namespace Memory
    {
        ...
        {
            int strLength = read<int>((void*)((uintptr_t)addr + Offsets::StringLength));

            if (strLength >= 16)
            {
                addr = read<void*>((void*)((uintptr_t)addr));
            }

            std::vector<char> buffer(256);
            SIZE_T bytesRead{ 0 };
            ReadProcessMemory(handle, addr, buffer.data(), strLength, &bytesRead);
    ...
    public:
        ...
        Instance(void* addr = nullptr) : address(addr) {}
        ...
        void* getPrimitive()
        {
            return Memory::read<void*>((void*)((uintptr_t)address + Offsets::Primitive));
        }

        Vector3 getPosition()
        {
            void* primitive = getPrimitive();
            return Memory::read<Vector3>((void*)((uintptr_t)primitive + Offsets::Position));
        }

        Instance getModelInstance()
        {
            return Instance(Memory::read<void*>((void*)((uintptr_t)address + Offsets::ModelInstance)));
        }
    ...
    {
    public:
        VisualEngine(void* addr = nullptr) : Instance(addr) {}
        ...
        {
            Vector4 quaternion;
            Vector2 dimensions{ (float)GetSystemMetrics(SM_CXSCREEN), (float)GetSystemMetrics(SM_CYSCREEN) };
    ...
    inline Instance getDataModel()
    {
        void* fakeDM = Memory::read<void*>((void*)((uintptr_t)Memory::getRobloxBaseAddr() + Offsets::FakeDataModelPointer));
        return Instance(Memory::read<void*>((void*)((uintptr_t)fakeDM + Offsets::FakeDataModelToDataModel)));
    }

    inline void setWalkSpeed(Instance humanoid, float speed)
    {
        Memory::write<float>((void*)((uintptr_t)humanoid.address + Offsets::WalkSpeed), speed);
        Memory::write<float>((void*)((uintptr_t)humanoid.address + Offsets::WalkSpeedCheck), speed);
    }

    inline void setJumpPower(Instance humanoid, float power)
    {
        Memory::write<float>((void*)((uintptr_t)humanoid.address + Offsets::JumpPower), power);
    }

    inline void setHealth(Instance humanoid, int health)
    {
        Memory::write<int>((void*)((uintptr_t)humanoid.address + Offsets::Health), health);
    }
}